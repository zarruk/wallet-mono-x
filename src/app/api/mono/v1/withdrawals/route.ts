...
export async function POST(request: Request) {
  try {
    const { amount, userId, accountId, description } = await request.json();
    ...
    const requestBody = {
      ...
      operation: "withdrawal", // Aquí está la diferencia clave con recharges
    ...
    if (!monoResponse.ok) {
      ...
      throw new Error(errorData.message || 'Error al procesar el retiro con Mono');
    }
    ...
    return NextResponse.json(
      { 
        error: error.message || 'Error al procesar el retiro',
